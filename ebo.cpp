...
int main()
{
	...
	unsigned int indices[] = { // note indices start from 0
	...
	// Now we can safely unbind VAO, VBO, and EBO
	// ORDER MATTERS HERE!! unbind the VAO first so it keeps
	// the binding info for the VBO and EBO
	glBindVertexArray(0);
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		// using the currently bound VAO and program
		glUseProgram(shaderProgram);
		// ONLY need to bind the VAO here, which will automatically
		// bind the corresponding VBO and EBO
		glBindVertexArray(VAO);
		//draw
